...
import { staffAuthAPI, getCurrentStaff } from '../api/auth';
import { useAuth } from '../context/AuthContext';

const ChangePassword = () => {
  const navigate = useNavigate();
  const { staff, updateStaff, logout, isAuthenticated } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await staffAuthAPI.changePassword(oldPassword, newPassword, confirmPassword);
      const updated = { ...getCurrentStaff(), must_change_password: false };
      updateStaff(updated);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          <h1 className="text-2xl font-bold text-primary-900 mb-6">
            {staff?.must_change_password ? 'Set New Password' : 'Change Password'}
          </h1>
