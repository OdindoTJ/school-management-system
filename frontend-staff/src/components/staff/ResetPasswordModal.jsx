...
const ResetPasswordModal = ({ isOpen, staff, onClose }) => {
  ...
  const handleReset = async () => {
    ...
    setError('');
    try {
      const res = await staffAPI.resetStaffPassword(staff.id);
        ...
        {!tempPassword ? (
          <>
            ...
            <p className="text-sm text-gray-600 mb-4">
              Generate a new temporary password for <strong>{staff.full_name}</strong>.
          ...
          <>
            ...
            <p className="text-sm text-gray-600 mb-4">
              New temporary password for <strong>{staff.full_name}</strong>:
            </p>
