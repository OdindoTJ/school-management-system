...
const DeactivateModal = ({ isOpen, staff, onClose, onSuccess }) => {
  ...
  const handleDeactivate = async () => {
    ...
    try {
      await staffAPI.deactivateStaff(staff.id, reason.trim());
      onSuccess();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        <h2 className="text-lg font-bold text-gray-900 mb-2">Deactivate Staff Member?</h2>
        <p className="text-sm text-gray-600 mb-4">
          <strong>{staff.full_name}</strong> will be deactivated. Their login will be
          disabled immediately, and all roles will be revoked.
        </p>
