  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [recordsRes, summaryRes] = await Promise.all([
        parentAPI.getChildAttendance(selectedChild.id, params),
        parentAPI.getChildAttendanceSummary(selectedChild.id, params),
      ]);

      const list = Array.isArray(recordsRes.data) ? recordsRes.data : recordsRes.data.results || [];
      setRecords(list);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">{selectedChild.full_name}'s attendance</p>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {records.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600 hidden md:table-cell">{r.reason || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
