...
import { studentAuthAPI, getCurrentStudent } from '../api/auth';
import { useAuth } from '../context/AuthContext';

const ChangePassword = () => {
  const navigate = useNavigate();
  const { student, updateStudent, logout, isAuthenticated } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await studentAuthAPI.changePassword(oldPassword, newPassword, confirmPassword);

      // Update student object (must_change_password becomes false)
      const updatedStudent = { ...getCurrentStudent(), must_change_password: false };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          <h1 className="text-2xl font-bold text-primary-900 mb-6">
            {student?.must_change_password ? 'Set New Password' : 'Change Password'}
          </h1>
