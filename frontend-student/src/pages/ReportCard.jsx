...
const ReportCard = () => {
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      try {
        const res = await studentAPI.getTerms();
  ...
  useEffect(() => {
    ...
    const fetchReportCard = async () => {
      setLoading(true);
      try {
        const res = await studentAPI.getReportCard(selectedTerm);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 no-print">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">View and download your term report</p>
        </div>
